...
def gen_users(n: int):
    users = [
        dict(
            ...
            description='社内検証用',
            raw_password = 'test',
            hashed_password = '$2a$10$6Yw4aufjz8wAgFCLXHJ/BugeJpZ6qY5ofvrfpWwJOLxgTg5eaq3l.',
            dark_mode=True
        )
    ]
    username_indexes = defaultdict(int)
    for user_id in range(2, n+1):
        # FIXME: id=1はtest001という特別なユーザ
...
def generate(users) -> str:
    ...
    with open('./initial-data/autogenerated_usernames.txt', 'w') as f:
        f.write('\n'.join(map(lambda u: u['name'], workload_users)) + '\n')
    with open('/tmp/user.go', 'w') as f:
        f.write(format_go(workload_users) + '\n')
