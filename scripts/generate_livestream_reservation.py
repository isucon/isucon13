...
GO_FORMAT="\tmustNewReservation({id}, {user_id}, \"{title}\", \"{description}\", \"{start_at}\", \"{end_at}\"),\n"
...
def dump(base_time, schedules, max_users, sqlpath, gopath):
    ...
    for idx, schedule in enumerate(schedules):
        ...
        for hours in random.sample(schedule, len(schedule)):
            user_id = (schedule_idx%max_users)+1
            ...
            sql_schedules.append(SQL_FORMAT.format(
                user_id=user_id,
                title=title,
                description=description,
                start_at=start_at.strftime("%Y-%m-%d %H:%M:%S"),
...
def get_args():
    ...
    parser.add_argument('--sqlpath', type=str, default='/tmp/livestream.sql', help='SQLファイル出力先パス')
...
def main():
    ...
    available_days = 365
    available_hours = (24*available_days)
    ...
    for schedule in schedules:
        assert(sum(schedule) == 24*available_days)

    base_time = datetime(2024, 4, 1, 0)
    dump(base_time, schedules, args.users, args.sqlpath, args.gopath)
