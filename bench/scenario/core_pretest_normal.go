...
func NormalPostLivecommentPretest(ctx context.Context, testUser *isupipe.User, dnsResolver *resolver.DNSResolver) error {
	...
	livecomments, err := client.GetLivecomments(ctx, livestream.ID, isupipe.WithLimitQueryParam(1))
	if err != nil {
		return err
	}
...
func NormalReportLivecommentPretest(ctx context.Context, dnsResolver *resolver.DNSResolver) error {
	...
	livecomments, err := client.GetLivecomments(ctx, livestream.ID, isupipe.WithLimitQueryParam(10))
	if err != nil {
		return err
	}
